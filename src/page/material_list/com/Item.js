...
export default function Item({ dataItem, dataSaved }) {
    ...
    const _handleDelAll = async () => {
        try {
            await apiClientMongo.delete('/master/all', {
                type: dataItem.optionKey
            });
            dispatch(fetchMasterData());
        } catch (err) {
        ...
        <CardCustom style={{ width: '30vw' }}>
            ...
            >
                ...
                <div>
                    <Popconfirm
                        ...
                        description={"It will delete all data!"}
                        onConfirm={_handleDelAll}
                        okText="Yes"
                        cancelText="No"
                    >
            ...
            {show ? (
                <div className='body'>
                    ...
                    <div style={{ maxHeight: '36vh', overflowY: 'scroll' }}>
                        {Array.isArray(data)
                            ? data.map((item) => (
                                <div
                                    style={{
