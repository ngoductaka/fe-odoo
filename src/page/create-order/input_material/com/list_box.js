...
import { delOne, getItem, insertObj, setItem, upsertOne } from 'helper/local_storage/storage';
...
import './App.scss';
import { ModalEditMul } from './edit_muiltiple';
import { AutoCompleteCus, Content, InputCus, SelectCus, selectS } from './styles';

const { Search } = Input;

const App = ({ input, handleSaveAll, handleAutoCheckBox }) => {
    ...
    const cellRendererSelect = React.useCallback(
        ({ cellData, rowData, column }) => {
            return (
                ...
                >
                    {_.get(masterData, [column.optionKey], []).map((item) => (
                        <Select.Option
            ...
            return (
                <DatePicker
                    ...
                    onChange={(val) => {
                        console.log('val', val);
                        _handleChangeFile(
                            `${rowData.id}**${column.dataKey}`,
                            moment(val).valueOf() / 1000
                        );
                    }}
                />
            );
        ...
        const normalRow = col.map((item) => {
            ...
            const _renderCell = (type) => {
                ...
                if (dataKey === 'warehouse_status') {
                    return ({ rowData, cellData }) => (
                        <Select
                            defaultValue={cellData}
                            onChange={(val) =>
                                _handleChangeFile(`${rowData.id}**${'warehouse_status'}`, val)
                            }
                            style={{ width: 130 }}
                        >
                ...
                if (type === 'typing') {
                    ...
                    return (props) => (
                        <CellRenderAutoComplete
                            cellData={props.cellData}
                            rowData={props.rowData}
                            column={props.column}
                            _handleChangeFile={_handleChangeFile}
                            // setShowStockMap={setShowStockMap}
                            locations={locations}
                        />
                    );
                } else if (type === 'fixed') {
                        ...
                        <Popover
                            ...
                            content={
                                <HeaderSearch
                                    ...
                                    dataOptions={_.get(masterData, [column.optionKey], []).map(
                                        (i) => ({ value: _.get(i, 'name', '') })
                                    )}
                                    type={type}
                                    _searchItem={(key, val) =>
                                        _searchItem(key, val, column?.filter)
                                    }
                                />
                            }
                            trigger='click'
    ...
    const selectRow = {
        ...
        cellRenderer: ({ rowData, column }) => {
            ...
            return (
                <Checkbox
                    checked={
                        selectedRowKeys === 'all' ? true : selectedRowKeys.includes(rowData.id)
                    }
                    onChange={(e) =>
                        setSelectedRowKeys(handleAddKeySelect(selectedRowKeys, rowData.id))
                    }
                />
            );
        },
    ...
    const _handleDuplicate = (dataRow, dataSource) => {
        Modal.confirm({
            ...
            content:
                'This action will duplicate material please make sure you update both item and save data',
            icon: <ExclamationCircleOutlined />,
            okText: 'Yes',
            okType: 'danger',
            cancelText: 'No',
    ...
    const columnsCal = [
        ...
        //                             <Menu.Item
        //                                 onClick={() => _handleDuplicate(rowData, dataSource)}
        //                             >
        //                                 Duplicate Data Row
        //                             </Menu.Item>
        //                         </Menu>
    ...
    const _handleStockout = (orderName) => {
        ...
        listOut.map((i) => {
            console.log('listLocation[i.location]', i.location, listLocation[i.location]);
        ...
        console.log('dataSavedataSavedataSavlistLocatione', listLocation);
        // out
    ...
    return (
        <div>
            ...
            >
                <div>
                    {count?.total ? (
                        <Button onClick={() => setStockOut(true)}>
            ...
            {loading ? (
                ...
                >
                    <div>
                        <LoadingOutlined
                            style={{ fontSize: '10vh', height: '10vh', marginBottom: 10 }}
                        />
                    </div>
                    loading...
                </div>
            ...
            <Modal
                title='Set material out'
                footer={null}
                visible={stockOut}
                onCancel={() => setStockOut(false)}
            >
...
const CellRenderAutoComplete = React.memo(
    ({ cellData, rowData, column, locations, _handleChangeFile, setShowStockMap }) => {
    ...
    const onChange = (val) => {
        try {
            if (!_.isEmpty(dataOptions)) {
                setOptions(
                    dataOptions.filter((i) => i?.value?.toLowerCase().includes(val?.toLowerCase()))
                );
            }
        } catch (err) {
